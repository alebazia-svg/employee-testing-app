...
- Внешний target настроен 2026-08-28: ежедневная копия шифруется открытым
  ключом и отправляется на Яндекс Диск, закрытый ключ остаётся на Mac владельца.
  Копия `offonika-portal-20260827T232844Z` была скачана обратно, её SHA-256 и
  внутренние контрольные суммы совпали, расшифровка прошла, дамп PostgreSQL
  содержит 506 восстанавливаемых объектов. Зашифрованная паролем recovery-копия
  закрытого ключа сохранена отдельно на Яндекс Диске; контрольная расшифровка и
  совпадение открытого ключа подтверждены. Пароль хранится в macOS Passwords.
...
| LB-9 | Закрыт 2026-08-28 | Предыдущие отказы были корректной защитой от неполного Platforma OFD; следующий полный прогон завершился успешно в 12:38 МСК. Найдена отдельная ложная тревога: watchdog во время долгого запуска показывал результат предыдущего запуска. | Watchdog теперь считает `active/activating` состоянием «проверка выполняется», а ошибку оценивает после завершения; контрольный запуск успешен. |
...
| LB-11 | Закрыт 2026-08-28 | Локальный backup/restore и зашифрованная внешняя копия на Яндекс Диске доказаны. Выполнены upload metadata readback, обратное скачивание, SHA-256, расшифровка и проверка дампа. | Контролировать ежедневную свежесть; закрытый ключ хранить отдельно от VPS. |
...
| FP-3 | Закрыт 2026-08-28: Attendance opens the current Moscow month, loads safe Google sources in parallel, normalizes known identities and explicitly labels Google as the temporary source. | Keep Google as the payroll attendance source during pilot. |
...
| FP-5 | Закрыт по решению владельца: the old credit issue is no longer employee-active and remains only as unresolved history. | Do not build a new escalation workflow for this historical case unless the business decision changes explicitly. |
