...
export function EmployeePaymentCheckActionCard(props: { reviewId: string; amountKopecks: number; bankOperationAt: string; initialResponse: string; latestAdminMessage?: string | null }) {
  ...
  return <Card className='rounded-[24px] border-amber-200 bg-amber-50'>
    <div className='flex gap-3'><span className='employee-material-icon flex h-11 w-11 shrink-0 items-center justify-center rounded-xl text-amber-700'><ReceiptText className='h-6 w-6' /></span><div><h1 className='text-xl font-black leading-snug'>Пробейте чек на {amount}</h1><p className='mt-2 text-sm font-bold leading-relaxed text-slate-700'>Терминал принял оплату в {time(props.bankOperationAt)}, но в 1С чека нет. Пробейте чек — сообщение исчезнет автоматически.</p></div></div>
    {handler ? <p className='mt-4 rounded-xl bg-white/80 px-3 py-2 text-sm font-bold text-slate-700'>Уже проверяет: {handler}</p> : null}
    {state === 'pending' && !handler ? <div className='mt-4 grid grid-cols-2 gap-2'><button type='button' disabled={busy} onClick={() => void act('help')} className='flex min-h-12 items-center justify-center gap-2 rounded-xl border border-amber-300 bg-white px-3 text-sm font-black text-slate-800 disabled:opacity-50'><HelpCircle className='h-4 w-4 shrink-0' />Не нахожу чек</button><button type='button' disabled={busy} onClick={() => void act('not_mine')} className='flex min-h-12 items-center justify-center gap-2 rounded-xl border border-amber-300 bg-white px-3 text-sm font-black text-slate-800 disabled:opacity-50'><UserRoundX className='h-4 w-4 shrink-0' />Не моя оплата</button></div> : null}
