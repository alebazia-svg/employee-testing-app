...
export function EmployeeAttentionSummaryCard({
  ...
  return <Card className={cn(
    ...
    blocked ? 'border-amber-200 bg-amber-50' : 'border-slate-200 bg-white',
  )}>
    <div className='flex min-h-[52px] items-center justify-between gap-3'>
      <div className='min-w-0'>
        <h2 className={cn('text-base font-black leading-tight', blocked ? 'text-amber-950' : 'text-slate-950')}>{title}</h2>
        <p className={cn('mt-1 text-xs font-semibold leading-snug', blocked ? 'text-amber-800' : 'text-slate-500')}>{subtitle}</p>
      </div>
