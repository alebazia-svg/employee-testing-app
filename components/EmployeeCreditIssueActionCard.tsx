...
import { HelpCircle, ReceiptText, UserRoundX } from 'lucide-react';
...
export function EmployeeCreditIssueActionCard(props: { issueId: number; title: string; instruction: string; notFoundLabel: string }) {
  ...
  return <Card className='rounded-[24px] border-amber-200 bg-amber-50'>
    <div className='flex gap-3'><span className='employee-material-icon flex h-11 w-11 shrink-0 items-center justify-center rounded-xl text-amber-700'><ReceiptText className='h-6 w-6' /></span><div><h1 className='text-xl font-black leading-snug'>{props.title}</h1><p className='mt-2 text-sm font-bold leading-relaxed text-slate-700'>{props.instruction} После этого задача исчезнет автоматически.</p></div></div>
    {state === 'normal' ? <button type='button' onClick={() => setState('exceptions')} className='mt-4 flex min-h-12 w-full items-center justify-center gap-2 rounded-xl border border-amber-300 bg-white px-3 text-sm font-black text-slate-800'><HelpCircle className='h-4 w-4 shrink-0' />Не могу исправить</button> : null}
    {state === 'exceptions' ? <div className='mt-4 grid grid-cols-2 gap-2'><button type='button' disabled={busy} onClick={() => void act('not_found')} className='flex min-h-12 items-center justify-center gap-2 rounded-xl border border-amber-300 bg-white px-3 text-sm font-black text-slate-800 disabled:opacity-50'><HelpCircle className='h-4 w-4 shrink-0' />{props.notFoundLabel}</button><button type='button' disabled={busy} onClick={() => void act('not_mine')} className='flex min-h-12 items-center justify-center gap-2 rounded-xl border border-amber-300 bg-white px-3 text-sm font-black text-slate-800 disabled:opacity-50'><UserRoundX className='h-4 w-4 shrink-0' />Продажа не моя</button></div> : null}
