...
export function ProcurementChangeHistory({events=[], buyerView=false}:{events?:PlanChangeEvent[]; buyerView?:boolean}) {
  const rows=events.filter(e=>['REVISION_REQUESTED','REVISION_APPROVED','REVISION_REJECTED','COMMENT_UPDATED','COMPLETED_WITHOUT_TOPUP','REOPENED'].includes(e.action)
    || e.action === 'UPDATED' && Boolean((e.snapshot as {revision?:{changes?:unknown[]}})?.revision?.changes?.length));
  if(!rows.length)return null;
  const titles:Record<string,string>={UPDATED:'Заявка изменена',REVISION_REQUESTED:'Изменения отправлены',REVISION_APPROVED:'Изменения согласованы',REVISION_REJECTED:'Оставлены прежние условия',COMMENT_UPDATED:'Комментарий изменён',COMPLETED_WITHOUT_TOPUP:'Завершена без доплаты',REOPENED:'Возвращена в активные'};
