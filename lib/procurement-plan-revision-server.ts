...
export async function decideApprovedRevision(id: string, actorId: number, revisionId: string, approve: boolean, reason: string) {
  ...
  return prisma.$transaction(async tx => {
    ...
    if (approve) {const paid = evidence?.get(id); if (!paid) throw new Error('Не удалось проверить оплату заявки.'); assertRevisionPaymentSafety(before, revision.data, paid);}
    const change = revision.data;
    ...
    const updated = await tx.supplierPaymentPlan.update({where:{id},data:{
      ...
      oneCCashEvidence:json({...object(before.oneCCashEvidence),pendingRevision:null,...(approve && identityChanged ? {paymentMatchFrom:revision.submittedAt}: {})}),
    }});
