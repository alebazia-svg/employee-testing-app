    ...
    for (const row of snapshot.rows) {
      ...
      const controlCase = await tx.creditRealizationControlCase.upsert({
        ...
        create: {
          ...
          paymentKind: row.result.paymentDocumentRef ? row.result.reasonCodes.includes('REQUIRED_CASH_RECEIPT_FISCAL_RECEIPT_MISSING') ? 'cash_receipt' : row.result.reasonCodes.includes('REQUIRED_ACQUIRING_FISCAL_RECEIPT_MISSING') ? 'acquiring' : row.links.cashReceipts.direct.some((item) => item.ref === row.result.paymentDocumentRef) ? 'cash_receipt' : 'acquiring' : null,
          paymentDocumentRef: row.result.paymentDocumentRef,
          paymentDocumentNumber: [...row.links.cashReceipts.direct, ...row.links.acquiring.direct].find((item) => item.ref === row.result.paymentDocumentRef)?.number || null,
          paymentAmountKopecks: row.result.paymentDocumentRef ? Math.round(row.result.expectedCurrentPayment * 100) : null,
          expectedCreditKopecks: Math.round(row.result.expectedCreditRemainder * 100),
