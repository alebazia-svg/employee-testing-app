...
export async function PATCH(req: Request) {
  if (process.env.ENABLE_DEV_WORKDAY_TOOLS !== 'true') return Response.json({ error: 'Dev workday tools are disabled' }, { status: 403 });
  ...
  const task = await prisma.shiftControlTask.findUnique({ where: { id: taskId }, include: { run: { include: { workDayEntry: true } } } });
