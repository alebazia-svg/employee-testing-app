...
export async function POST(_: Request, context: { params: { id: string } }) {
  ...
  const caseRow = await prisma.expenseRequestAdminCase.findUnique({ where: { id: context.params.id }, select: { oneCRequestRef: true } });
  ...
  const result = await prisma.$transaction(async (db) => {
    const viewed = await db.expenseRequestAdminCase.updateMany({
      where: { id: context.params.id, seenAt: null },
