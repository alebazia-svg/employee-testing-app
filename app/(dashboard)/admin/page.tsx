...
export default async function AdminPage() {
  ...
  const [employees, schedules, vacations, workdays, issues, reviews, closeRequests, expenseCases, cashOperationErrors, terminalSummary] = await Promise.all([
    ...
    prisma.terminalFiscalEmployeeReview.findMany({
      ...
      select: { id: true, status: true, assignmentScope: true, amountKopecks: true, bankOperationAt: true, updatedAt: true, employee: { select: { name: true } }, messages: { take: 1, orderBy: { createdAt: 'desc' }, select: { createdAt: true, author: { select: { role: true } } } } },
  ...
  const actions: ActionItem[] = [
    ...
    ...reviews.filter((item) => item.status === 'admin_review' || item.messages[0]?.author.role === 'EMPLOYEE').map((item) => ({ key: `review-${item.id}`, title: item.status === 'admin_review' ? `Нужна проверка администратора · ${item.assignmentScope !== 'individual' ? 'сотрудники Розницы' : item.employee.name}` : `Сообщение · ${item.assignmentScope !== 'individual' ? 'сотрудники Розницы' : item.employee.name}`, detail: `Продажа ${time(item.bankOperationAt)} · ${(item.amountKopecks / 100).toLocaleString('ru-RU')} ₽`, href: `/admin/workday/payment-checks/${item.id}`, occurredAt: item.messages[0]?.createdAt ?? item.updatedAt, kind: item.status === 'admin_review' ? 'decision' as const : 'message' as const })),
  ...
  const employeeProblems = [
    ...
    ...reviews.filter((item) => item.status === 'open').map((item) => ({ key: `review-${item.id}`, title: 'Проверить продажу', detail: `${item.assignmentScope !== 'individual' ? 'Сотрудники Розницы' : item.employee.name} · ${time(item.bankOperationAt)} · ${(item.amountKopecks / 100).toLocaleString('ru-RU')} ₽`, href: `/admin/workday/payment-checks/${item.id}`, occurredAt: item.updatedAt })),
