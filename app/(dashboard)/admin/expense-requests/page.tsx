    ...
    <AdminShell>
      ...
      <div className='mt-3 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between'>
        <div>
          ...
          <p className='mt-1 text-sm font-medium text-slate-500'>Текущие заявки из 1С. Просмотр и сохранение вывода не меняют заявку в 1С.</p>
        </div>
      ...
      <section className='mt-5 grid gap-3 sm:grid-cols-3'>
        {[
          ...
          { label: 'Вывод сохранён', value: reviewedCount, icon: CheckCircle2, tone: 'text-green-700 bg-green-50' },
      ...
      <section className='mt-5 overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200/80'>
        <div className='border-b border-slate-200 px-5 py-4'>
          <div className='flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between'>
            <div><h2 className='text-lg font-extrabold text-slate-950'>{view === 'current' ? 'Не согласованы в 1С' : view === 'reviewed' ? 'С сохранённым выводом' : 'История всех заявок'}</h2><p className='mt-1 text-xs font-medium text-slate-500'>Новые находятся сверху. Подсказки предназначены только для администратора.</p></div>
            <div className='flex flex-wrap gap-2'>
              {[
                { key: 'current', label: 'Текущие', href: '/admin/expense-requests' },
                { key: 'reviewed', label: 'С выводом', href: '/admin/expense-requests?view=reviewed' },
