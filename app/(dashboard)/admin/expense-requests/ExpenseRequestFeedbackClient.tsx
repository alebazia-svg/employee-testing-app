...
const decisions = [
  { key: 'normal', label: 'Всё нормально' },
  { key: 'clarification_required', label: 'Действительно нужно уточнить' },
  { key: 'hint_unnecessary', label: 'Подсказка лишняя' },
  { key: 'rule_change_required', label: 'Правило нужно изменить' },
] as const;
...
export function ExpenseRequestFeedbackClient({ caseId, reasonCodes }: { caseId: string; reasonCodes: string[] }) {
  ...
  return (
    <div className='space-y-4'>
      ...
      <Button type='button' disabled={saving} onClick={() => void save('overall')}>{saving ? 'Сохраняем…' : 'Сохранить вывод'}</Button>
