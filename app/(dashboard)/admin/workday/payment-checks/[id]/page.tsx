...
export default async function AdminPaymentCheckPage({ params }: { params: { id: string } }) {
  ...
  const match = await prisma.terminalFiscalMatch.findUnique({
    ...
    select: { status: true, reasonCode: true, oneCSourceRef: true, timeDifferenceSeconds: true },
  });
    ...
    <AdminShell>
      ...
      <div className='grid gap-5 xl:grid-cols-[minmax(0,0.85fr)_minmax(360px,1.15fr)]'>
        <Card className={resolved ? 'border-green-200 bg-green-50' : 'border-amber-200 bg-amber-50'}>
          ...
          <dl className={`mt-5 grid gap-3 border-t pt-4 text-sm sm:grid-cols-2 ${resolved ? 'border-green-200' : 'border-amber-200'}`}><div><dt className={`font-semibold ${resolved ? 'text-green-700' : 'text-amber-700'}`}>Сотрудник</dt><dd className={`mt-1 font-extrabold ${resolved ? 'text-green-950' : 'text-amber-950'}`}>{review.employee.name}</dd></div><div><dt className={`font-semibold ${resolved ? 'text-green-700' : 'text-amber-700'}`}>Основание адресации</dt><dd className={`mt-1 font-extrabold ${resolved ? 'text-green-950' : 'text-amber-950'}`}>Единственный кассир рядом по этой ККМ</dd></div></dl>
          {!resolved && <p className='mt-4 text-xs font-semibold leading-relaxed text-amber-800'>Это нейтральная проверка, а не подтверждённая ошибка сотрудника. Рабочее место и оператор ОФД для определения сотрудника не используются.</p>}
        </Card>
        <Card>
