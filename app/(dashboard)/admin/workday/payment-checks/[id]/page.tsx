    ...
    <AdminShell>
      ...
      <div className='grid gap-5 xl:grid-cols-[minmax(0,0.85fr)_minmax(360px,1.15fr)]'>
        <Card className={resolved ? 'border-green-200 bg-green-50' : 'border-amber-200 bg-amber-50'}>
          ...
          <dl className={`mt-5 grid gap-3 border-t pt-4 text-sm sm:grid-cols-2 ${resolved ? 'border-green-200' : 'border-amber-200'}`}><div><dt className={`font-semibold ${resolved ? 'text-green-700' : 'text-amber-700'}`}>{shared ? 'Ответственные сотрудники' : 'Сотрудник'}</dt><dd className={`mt-1 font-extrabold ${resolved ? 'text-green-950' : 'text-amber-950'}`}>{shared ? review.participants.map((item) => item.user.name).join(', ') : review.employee.name}</dd></div><div><dt className={`font-semibold ${resolved ? 'text-green-700' : 'text-amber-700'}`}>Основание адресации</dt><dd className={`mt-1 font-extrabold ${resolved ? 'text-green-950' : 'text-amber-950'}`}>{review.assignmentScope === 'retail_shift' ? 'Работали в Рознице в момент оплаты' : review.assignmentScope === 'retail_day' ? 'Работали в Рознице в этот день' : 'Кассир по данным 1С'}</dd></div></dl>
          {!resolved && <p className='mt-4 text-xs font-semibold leading-relaxed text-amber-800'>{shared ? 'Это общая задача сотрудникам Розницы, а не персональное обвинение. Она закроется у всех после появления чека в 1С.' : 'Это нейтральная проверка, а не подтверждённая ошибка сотрудника.'}</p>}
