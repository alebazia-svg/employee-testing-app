...
import { departmentLabel, formatDateLabel, formatTime, getMoscowDateKey, scheduleStatusLabel, workDayStatusLabel } from '@/lib/workday';
...
export default async function AdminWorkdayPage() {
  ...
  const [employees, schedules, workDays, unfinishedWorkDays] = await Promise.all([
    prisma.user.findMany({
      where: { role: 'EMPLOYEE', isActive: true },
      orderBy: [{ department: 'asc' }, { name: 'asc' }],
      select: { id: true, name: true, department: true },
    }),
    prisma.workScheduleEntry.findMany({ where: { date: today } }),
    prisma.workDayEntry.findMany({ where: { date: today } }),
    prisma.workDayEntry.findMany({
      where: { status: { in: ['active', 'missing_checkout'] }, endedAt: null, date: { not: today } },
  ...
  const missingCheckoutToday = workDays.filter((entry) => entry.status === 'active' && !entry.endedAt).length;
  const missingCheckoutCount = unfinishedWorkDays.length + missingCheckoutToday;
    ...
    <AdminShell>
      <div className='space-y-6'>
        <div className='flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between'>
          ...
          <Badge className='w-fit bg-white text-slate-700 ring-1 ring-slate-200'>{formatDateLabel(today)}</Badge>
        </div>
        ...
        {unfinishedWorkDays.length > 0 && (
          <Card className='border-amber-200 bg-amber-50'>
            <div className='flex items-start gap-3'>
              ...
              <div>
                <p className='font-extrabold text-amber-950'>Есть незавершённые рабочие дни раньше сегодняшнего дня</p>
        ...
        <Card className='p-0'>
          <div className='border-b border-slate-200 px-5 py-4'>
            <h2 className='text-lg font-extrabold text-slate-950'>Сегодня по сотрудникам</h2>
